...
export default EditBiometric = (props) => {
    const navigation = useNavigation();
    const [height, setHeight] = useState(props.currentUser.height);
    const [weight, setWeight] = useState(props.currentUser.weight);
    const [age, setAge] = useState(props.currentUser.age);
    const [gender, setSelectedGender] = useState(props.currentUser.gender);
    const [showOptions, setShowOptions] = useState(false);
    const [weightLoss, setIsWeightLoss] = useState(JSON.parse(props.currentUser.weightLoss));
    const [muscleGain, setIsMuscleGain] = useState(JSON.parse(props.currentUser.muscleGain));
        ...
        if (errors == false) {
            try {
                ...
                if (userResponse === undefined) {
                    alert("Sorry, couldn't update your biometrics")

                }
                else {
                    console.log("Successfully Updated Biometrics!");

                    navigation.navigate("Home", { currentUser: currentUser });
                }
            } catch (error) {
                setErrorMessage("can not edit biometric info");
    ...
    return (
        ...
        <ScrollView style={styles.container}>
            <Text style={styles.header}>Update Biometric Details</Text>
